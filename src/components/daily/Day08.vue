...
const initThreeJs = () => {
  ...
  controls.maxPolarAngle = Math.PI / 2 - 0.1 // 禁止钻入地下
...
const update = (delta) => {
  ...
  // 旋转星空，增加动态感
  if (starField) starField.rotation.y += delta * 0.02

  // --- 实验 H 核心：实时更新幽灵火位置 ---
  if (ghostFire) {
    const time = clock.getElapsedTime()
    const positions = ghostFire.geometry.attributes.position.array

    for (let i = 0; i < 200; i++) {
      const i3 = i * 3
      // y 轴位置 = 初始位置 + sin(时间 + 偏移)
      // 使用 i 作为一个随机偏移，让每个点跳动的节奏都不一样
      positions[i3 + 1] = firePositions[i3 + 1] + Math.sin(time + i) * 0.2
    }
    // 关键：告诉 GPU 数据变了，重新读取！
  ...
  // --- 实验 I 核心：点阵化模型的抖动动画 ---
  if (particleKnot) {
    particleKnot.rotation.z += delta * 0.1
    particleKnot.rotation.y += delta * 0.1

    const positions = particleKnot.geometry.attributes.position.array
    for (let i = 0; i < positions.length; i++) {
      // 给每个坐标加一个极其微小的随机抖动，让它看起来像在闪烁
      positions[i] += (Math.random() - 0.5) * 0.01
    }
    particleKnot.geometry.attributes.position.needsUpdate = true
  }
...
const createStarfield = () => {
  ...
  const material = new THREE.PointsMaterial({
    ...
    map: starTexture,
    transparent: true,
    blending: THREE.AdditiveBlending, // 开启混合
...
// --- 实验 H：幽灵火 ---
let ghostFire = null
let firePositions = null // 我们需要保存原始坐标，用来做位移基准
const createGhostFire = () => {
  const count = 200
  firePositions = new Float32Array(count * 3)

  for (let i = 0; i < count; i++) {
    const i3 = i * 3
    // 幽灵火主要分布在地面附近 (y 从 0.2 到 2 之间)
    firePositions[i3] = (Math.random() - 0.5) * 15
    firePositions[i3 + 1] = Math.random() * 2 + 0.2
    firePositions[i3 + 2] = (Math.random() - 0.5) * 15
  }

  const geometry = new THREE.BufferGeometry()
  geometry.setAttribute('position', new THREE.BufferAttribute(firePositions, 3))
  ...
  ghostFire = new THREE.Points(geometry, material)
  scene.add(ghostFire)
}

// --- 实验 I：点阵化模型 ---
let particleKnot = null
const createParticleKnot = () => {
  // 1. 创建一个复杂的几何体 (圆环结)
  const tempGeom = new THREE.TorusKnotGeometry(1, 0.3, 100, 16)

  // 2. 提取它的顶点数据
  // 我们直接白嫖这个几何体算好的 position 属性
  const positions = tempGeom.attributes.position.clone()
  ...
  particleKnot = new THREE.Points(geometry, material)
  particleKnot.position.set(0, 3, -5) // 悬浮在后方空中
